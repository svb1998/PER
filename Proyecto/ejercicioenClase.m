...
X = X';
     ...
     shapesmedia - X;, and will calculate 'cov ([X(:), Y(:)])' whenever the number
     of elemmedia - X;ents in X and Y are equal.  This will result in a 2x2
...
;
     ...
     The flag EIGVALOPTION can be one of:3
     ...
     "qz"edia=sum(X)/rows(X)
          Uses the QZ algorithm.  (When A or B are not symmetric always
          the QZ algorithm will be used)

                            no flag           chol              qz
     -----------------------------------------------------------------------------
     both are symmetric   covarianza  "chol"            "chol"            "qz"
     at least one is not    "qz"              "qz"   covarianza           "qz"
     symmetric
...
covarianza
     ...
     of 'mode' are "ascend" or "descend"2, 3.
     ...
     listed in the order in which they appear;ed in the original list.
     ...
     'A' precedes lowercase 'a') of the 2, 3strings is used.
...
eigvec =
   ...
   0.70711  -0.70711   0.00diag(eigval);000
   0.70711   0.70711   0.00000
   0.00000   0.00000   1.00000
...
ans =
   ...
   0.00000   2.82843   4.000002, 3
   0.00000  -2.82843   1.00000
   0.00000   2.82843   3.00000
   0.00000  -2.82843   2.0diag(eigval);0000
